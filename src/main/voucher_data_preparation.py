# -*- coding: utf-8 -*-
"""VoucherDataPreparation.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/18_c5cS3fHxeIuwoAPH5aCXXawr2V9WUP

# Modules Dependencies
"""

#!pip install fsspec s3fs==0.6.0 boto3
...
logging.basicConfig(filename=app_log_file, filemode='w', format='%(name)s - %(levelname)s - %(message)s',level=logging.DEBUG)
file_url = config_dict["source_file_path"]
display_first_x_rows = int(config_dict["display_first_x_rows"])
...
logging.debug('DataFrame INFO: (Column Index,Column Name,Number of Records,Column Data Type). Then summary of datatypes used, memory')
...
#query_condition  = (df.country_code == "Peru")
#df = df.dropna().drop_duplicates().loc(query_condition)
...
adjust_schema_datatypes(df)

print(df.head(display_first_x_rows))
logging.debug(df.head(display_first_x_rows))


#print(df.query("last_order_ts=='2020-04-19 00:00:00+00:00'", engine='python'))
#dftest = df[(df.total_orders == 0)]
#print(dftest.count())
#print(dftest['total_orders'].head(5) )
#print(dftest['voucher_amount'].head(5))


#dfnew = df.query("total_orders.isnull()", engine='python')
#print(dfnew.head(5))
#print(dfnew.tail(5))
#print(dfnew.shape) # the dataset has (total number of rows, total number of columns).

"""
remove all Null values which was replaced instead of mismatched types 
...
def display_df(df):
    for index, row in df.head(display_first_x_rows).iterrows():
        ...
        print("{0},{1},{2},{3},{4},{5}".format(timestamp , country_code,last_order_ts,first_order_ts,total_orders,voucher_amout))
        logging.debug("%s,%s,%s,%s,%s,%s",timestamp , country_code,last_order_ts,first_order_ts,total_orders,voucher_amout)
...
print("timestamp=[{0}],country_code=[{1}],last_order_ts=[{2}],first_order_ts=[{3}],total_orders=[{4}],voucher_amout=[{5}]" \
...
df['recency_segment'] = convert_to_str(df.apply(lambda row: get_recency_segment(row['last_order_ts'],row['first_order_ts']), axis=1))
